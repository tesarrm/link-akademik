...
frappe.ui.form.on('Student Attendance Tool', {
	...
	student_group: function(frm) {
		if ((frm.doc.student_group && frm.doc.date) || frm.doc.course_schedule) {
			...
			var method = "akademik.akademik.doctype.student_attendance_tool.student_attendance_tool.get_student_attendance_records";
	...
	get_students: function(frm, students) {
		students = students || [];
		frm.students_editor = new akademik.StudentsEditor(frm, frm.students_area, students);
	}
});


akademik.StudentsEditor = class StudentsEditor {
				...
				frappe.confirm(__("Do you want to update attendance? <br> Present: {0} <br> Absent: {1}",
					...
					function() {	//ifyes
						if(!frappe.request.ajax_count) {
							frappe.call({
								method: "akademik.akademik.api.mark_attendance",
								freeze: true,
